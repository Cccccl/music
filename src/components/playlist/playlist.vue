        ...
        <scroll class="list-content" ref="listContent" :refreshDelay="refreshDelay" :data="sequenceList">
          <transition-group name="list" tag="ul">
            <li class="item" v-for="(item, index) in sequenceList" :key="item.id"
              ref="listItem" @click="selectItem(item, index)"
            >
              <i class="current" v-bind:class="getCurrentIcon(item)"></i>
...
export default {
  ...
  methods: {
    ...
    scrollToCurrent: function (current) {
      const index = this.sequenceList.findIndex((song) => {
        return current.id === song.id
      })
      this.$refs.listContent.scrollToElement(this.$refs.listItem[index], 50)
    },
